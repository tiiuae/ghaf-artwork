...
auto GetDeviceName(const IAudioControlBackend::IDevice& device)
{
    ...
    if (auto* defaultable = dynamic_cast<const IAudioControlBackend::IDefaultable*>(&device))
        isDefault = defaultable->isDefault();

    // Set description as a name for sinks and sources -- as it's less ugly
    const auto name = (device.getType() == IAudioControlBackend::IDevice::Type::Sink || device.getType() == IAudioControlBackend::IDevice::Type::Source)
